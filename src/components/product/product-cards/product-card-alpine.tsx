...
function RenderPopupOrAddToCart({
  ...
  props: { data: Product; lang: string };
}) {
  let { data, lang } = props;
  // console.log(variant);
  ...
  if (product_type === 'variable') {
    return (
      <button
        ...
        aria-label={t('text-view') ?? 'View product'}
      >
    ...
    >
      <div className="relative shrink-0">
        <div className="overflow-hidden mx-auto w-full sm:w-[180px] h-[180px] md:w-[200px] md:h-[200px] transition duration-200 ease-in-out transform group-hover:scale-105 relative">
          <Image
            src={image?.thumbnail || image?.original || productPlaceholder}
        ...
        <div className="w-full h-full absolute top-0 pt-2.5 md:pt-3.5 px-3 md:px-4 lg:px-[18px] z-10 -mx-0.5 sm:-mx-1">
          ...
          <div className={`block product-count-button-position`}>
            <RenderPopupOrAddToCart props={{ data: product, lang }} />
          </div>
        </div>
      </div>
